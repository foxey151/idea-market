    ...
    if (error && typeof error === 'object') {
      if ('message' in error) {
        ...
        if (
          ...
          errorMessage =
            'Storageへのアクセス権限がありません。ログインしてから再度お試しください。';
        } else if (
          ...
          errorMessage =
            'ファイルサイズが大きすぎます。最大20MBまでのファイルを選択してください。';
        } else if (
          errorMessage.includes('Invalid bucket') ||
          errorMessage.includes('not found')
        ) {
          errorMessage =
            'ストレージの設定に問題があります。管理者にお問い合わせください。';
        } else if (
...
export const uploadFiles = async (
  ...
  try {
    ...
    if (errors.length > 0) {
      ...
      throw new Error(
        `${errors.length}/${files.length}件のファイルアップロードに失敗: ${errorMessages}`
      );
    }
