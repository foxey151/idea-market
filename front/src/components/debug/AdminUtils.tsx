...
export function AdminUtils() {
  ...
  const makeUserAdmin = async () => {
    if (!userId && !email) {
      toast({
        title: 'エラー',
        description: 'ユーザーIDまたはメールアドレスのいずれかを入力してください',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      if (userId) {
        ...
        const targetUser = users?.users.find((user: { email?: string }) => user.email === email);
      ...
      toast({
        title: 'エラー',
        description: error instanceof Error ? error.message : '管理者設定に失敗しました',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const createProfile = async () => {
    ...
    try {
      ...
      toast({
        title: 'エラー',
        description: error instanceof Error ? error.message : 'プロファイル作成に失敗しました',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
    ...
    <Card className="w-full max-w-4xl mx-auto">
      ...
      <CardContent className="space-y-6">
        <div className="grid gap-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="email">メールアドレス (ユーザーID未入力時に使用)</Label>
            <Input
              id="email"
              type="email"
        ...
        <div className="text-sm text-muted-foreground space-y-2">
          ...
          <ul className="list-disc pl-6 space-y-1">
            <li><strong>管理者に設定:</strong> 既存ユーザーのロールを'admin'に変更</li>
            <li><strong>プロファイル作成:</strong> プロファイルが存在しない場合に新規作成</li>
