  ...
  const handleEmailSignup = async (data: SignupFormData) => {
    try {
      ...
      if (error) {
        ...
        toast({
          title: "登録完了",
          description: "確認メールを送信しました。メールボックスをご確認ください。",
          duration: 5000
        })
        onOpenChange(false)
        resetForm()
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-center">
            {signupMethod === 'select' ? '新規登録' : 'メールで新規登録'}
          </DialogTitle>
          <DialogDescription className="text-center">
            {signupMethod === 'select' 
              ? 'アカウントを作成して、アイデアマーケットを始めましょう'
              : 'アカウント情報を入力してください'
            }
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={handleSubmit(handleEmailSignup)} className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="privacyAgreed"
                  onCheckedChange={(checked) => setValue("privacyAgreed", !!checked)}
                />
              ...
              {errors.privacyAgreed && (
                <p className="text-sm text-destructive">{errors.privacyAgreed.message}</p>
              )}
            </div>
