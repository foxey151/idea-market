...
export function DocumentEditor({ open, onOpenChange, documentType }: DocumentEditorProps) {
  ...
  const fetchDocumentContent = async (type: string) => {
    try {
      ...
      // 対応するページのHTMLを取得
      const response = await fetch(documentPaths[type as keyof typeof documentPaths])
      if (!response.ok) {
      ...
      toast({
        title: "エラー",
        description: "文書の取得に失敗しました。デフォルトコンテンツを表示します。",
        variant: "destructive"
      })
...
従業員数: はははははははははははははははははははははははははははははははははははははははははははははははははははははははははははは`
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        title: "保存完了",
        description: result.message || `${documentTitles[documentType]}を保存しました。`,
      })
      ...
      toast({
        title: "保存エラー",
        description: error instanceof Error ? error.message : "文書の保存に失敗しました。",
        variant: "destructive"
      })
    } finally {
      setSaving(false)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-2xl">{documentTitles[documentType]}の編集</DialogTitle>
          <DialogDescription>
